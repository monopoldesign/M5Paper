...
void key_mono_cb(epdgui_args_vector_t &args)
{
	Frame_Base *frame = EPDGUI_GetFrame("Frame_Mono");
	if(frame == NULL)
	{
		frame = new Frame_Mono();
		EPDGUI_AddFrame("Frame_Mono", frame);
	}
	EPDGUI_PushFrame(frame);
	*((int*)(args[0])) = 0;
}

void key_list_cb(epdgui_args_vector_t &args)
{
	Frame_Base *frame = EPDGUI_GetFrame("Frame_List");
	if(frame == NULL)
	{
		frame = new Frame_List();
		EPDGUI_AddFrame("Frame_List", frame);
	}
	EPDGUI_PushFrame(frame);
	*((int*)(args[0])) = 0;
}

void key_basic_cb(epdgui_args_vector_t &args)
{
	Frame_Base *frame = EPDGUI_GetFrame("Frame_Basic");
	if(frame == NULL)
	{
		frame = new Frame_Basic();
		EPDGUI_AddFrame("Frame_Basic", frame);
	}
	EPDGUI_PushFrame(frame);
	*((int*)(args[0])) = 0;
}
...
Frame_Main::Frame_Main(void): Frame_Base(false)
{
	...
	_key[8] = new EPDGUI_Button("测试", 20 + 0 * 136, 390, KEY_W, KEY_H);
	_key[9] = new EPDGUI_Button("测试", 20 + 1 * 136, 390, KEY_W, KEY_H);
	_key[10] = new EPDGUI_Button("测试", 20 + 2 * 136, 390, KEY_W, KEY_H);
	...
	*(_key[kKeyHome]->CanvasPressed()) = *(_key[kKeyHome]->CanvasNormal());
	...
	_key[kKeyMono]->CanvasNormal()->pushImage(0, 0, 92, 92, ImageResource_main_icon_sdcard_92x92);
	*(_key[kKeyMono]->CanvasPressed()) = *(_key[kKeyMono]->CanvasNormal());
	_key[kKeyMono]->CanvasPressed()->ReverseColor();
	_key[kKeyMono]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, (void*)(&_is_run));
	_key[kKeyMono]->Bind(EPDGUI_Button::EVENT_RELEASED, key_mono_cb);

	_key[kKeyList]->CanvasNormal()->pushImage(0, 0, 92, 92, ImageResource_main_icon_nounList_92x92);
	*(_key[kKeyList]->CanvasPressed()) = *(_key[kKeyList]->CanvasNormal());
	_key[kKeyList]->CanvasPressed()->ReverseColor();
	_key[kKeyList]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, (void*)(&_is_run));
	_key[kKeyList]->Bind(EPDGUI_Button::EVENT_RELEASED, key_list_cb);

	_key[kKeyBasic]->CanvasNormal()->pushImage(0, 0, 92, 92, ImageResource_main_icon_nounCode_92x92);
	*(_key[kKeyBasic]->CanvasPressed()) = *(_key[kKeyBasic]->CanvasNormal());
	_key[kKeyBasic]->CanvasPressed()->ReverseColor();
	_key[kKeyBasic]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, (void*)(&_is_run));
	_key[kKeyBasic]->Bind(EPDGUI_Button::EVENT_RELEASED, key_basic_cb);
...
void Frame_Main::AppName(m5epd_update_mode_t mode)
{
	...
	_names->fillCanvas(0);
    _names->drawString("MoNo", 20 + 46 + (0 * 136), 16);
    _names->drawString("List", 20 + 46 + (1 * 136), 16);
    _names->drawString("Basic", 20 + 46 + (2 * 136), 16);
