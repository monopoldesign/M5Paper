...
void Frame_Base::menubtn(String title, uint16_t width)
{
	_key_menu = new EPDGUI_Button(382, 12, width, 48);
	_key_menu->CanvasNormal()->fillCanvas(0);
	_key_menu->CanvasNormal()->setTextSize(26);
	_key_menu->CanvasNormal()->setTextDatum(CL_DATUM);
	_key_menu->CanvasNormal()->setTextColor(15);
	_key_menu->CanvasNormal()->drawString(title, 47 + 13, 28);
	_key_menu->CanvasNormal()->pushImage(15, 8, 32, 32, ImageResource_item_icon_arrow_r_32x32);
	*(_key_menu->CanvasPressed()) = *(_key_menu->CanvasNormal());
	_key_menu->CanvasPressed()->ReverseColor();
}
